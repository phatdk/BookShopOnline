...
namespace BookShop.Web.Blazor.Service
{
	public class Collection_BookService
	{
		...
		public async Task<Collection_BookVM?> AddAsync(Collection_BookVM item)
		{
			var response = await _httpClient.PostAsJsonAsync(_url + $"add", item);
			return await response.Content.ReadFromJsonAsync<Collection_BookVM>();
		}
		
		public async Task<Collection_BookVM?> UpdateAsync(Collection_BookVM item)
		{
			var response = await _httpClient.PutAsJsonAsync(_url + $"update/{item.Id}", item);
			return await response.Content.ReadFromJsonAsync<Collection_BookVM>();
		}
		
		public async Task<Collection_BookVM?> AddAsync(Guid id)
		{
